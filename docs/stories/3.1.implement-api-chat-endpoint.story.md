...
- [ ] Integration tests with Ollama SDK against running server (AC: 1-12)
  - [ ] Create tests/integration/test_ollama_sdk_chat.py
  - [ ] Set up test fixture to start actual server instance
  - [ ] Test client.chat() with basic conversation against live server
  - [ ] Test streaming chat responses with real-time validation
  - [ ] Test multi-turn conversations with context preservation
  - [ ] Test with system prompts and role handling
  - [ ] Test parameter translation (temperature, top_p, max_tokens)
  - [ ] Verify response format matches SDK expectations
  - [ ] Test error scenarios with running server
  - [ ] Performance test to ensure minimal translation overhead
- [ ] API compliance validation (AC: 1-6)
  - [ ] Run contract tests to verify compliance
  - [ ] Test with Ollama CLI: ollama chat
...
- Modified: src/ollama_openai_proxy/services/enhanced_translation_service.py (added chat translation methods)
