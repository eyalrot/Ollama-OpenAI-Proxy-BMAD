...
class OpenAIService:
    ...
    def client(self) -> AsyncOpenAI:
        ...
        if self._client is None:
            ...
            http_client = httpx.AsyncClient(
                timeout=httpx.Timeout(timeout=float(self.settings.request_timeout), connect=10.0),
            ...
            logger.info(
                ...
                extra={"base_url": self.settings.openai_api_base_url, "timeout": self.settings.request_timeout},
            )
    ...
    def _should_retry(self, error: Exception) -> bool:
        ...
        # Retry on 5xx errors
        if isinstance(error, APIError) and hasattr(error, "status_code") and error.status_code:
            return bool(500 <= error.status_code < 600)
        ...
        while attempt <= self.max_retries:
            try:
                ...
                logger.info(
                    ...
                    extra={"request_id": request_id, "duration_ms": round(duration * 1000), "attempt": attempt + 1},
                )
    ...
    async def list_models(self) -> List[Model]:
        ...
        async def _list() -> List[Model]:
            response = await self.client.models.list()
            return list(response.data)

        return cast(List[Model], await self._execute_with_retry("list_models", _list))

    async def create_chat_completion(
        self, model: str, messages: List[Dict[str, str]], stream: bool = False, **kwargs: Any
    ) -> ChatCompletion:
        """
        ...
        return cast(ChatCompletion, await self._execute_with_retry("create_chat_completion", _create))
    ...
    async def create_chat_completion_stream(
        ...
        logger.info("Starting streaming chat completion", extra={"request_id": request_id, "model": model})
        ...
        try:
            ...
            logger.info("Completed streaming chat completion", extra={"request_id": request_id, "chunks": chunk_count})
    ...
    async def create_embedding(self, model: str, input: str, **kwargs: Any) -> CreateEmbeddingResponse:
        ...
        async def _create() -> CreateEmbeddingResponse:
            return await self.client.embeddings.create(model=model, input=input, **kwargs)

        return cast(CreateEmbeddingResponse, await self._execute_with_retry("create_embedding", _create))
