...
import pytest
from ollama_openai_proxy.services.enhanced_translation_service import EnhancedTranslationService, ModelRegistry
...
class TestEnhancedTranslationService:
    ...
    def test_estimate_model_size_known_model(self):
        ...
        model = Model(id="gpt-4", created=1234567890, object="model", owned_by="openai")
    ...
    def test_estimate_model_size_unknown_model(self):
        ...
        # Embedding model
        embed_model = Model(id="new-embedding-model", created=1234567890, object="model", owned_by="openai")
        size = EnhancedTranslationService.estimate_model_size(embed_model)
        assert size == 500_000_000

        # GPT-4 variant
        gpt4_model = Model(id="gpt-4-vision", created=1234567890, object="model", owned_by="openai")
        size = EnhancedTranslationService.estimate_model_size(gpt4_model)
        assert size == 20_000_000_000

        # Unknown model
        unknown_model = Model(id="future-model", created=1234567890, object="model", owned_by="openai")
    ...
    def test_create_ollama_model_with_options(self):
        ...
        # Without digest
        model1 = EnhancedTranslationService.create_ollama_model(openai_model, include_digest=False)
        assert model1.digest == ""

        # With digest
        model2 = EnhancedTranslationService.create_ollama_model(openai_model, include_digest=True)
        assert model2.digest.startswith("sha256:")

        # With custom metadata (Note: base model doesn't have these fields)
        model3 = EnhancedTranslationService.create_ollama_model(openai_model, custom_metadata={"size": 999})
    ...
    def test_translate_with_metadata(self):
        ...
        # Without metadata
        response1 = EnhancedTranslationService.translate_with_metadata(models, include_metadata=False)
        assert len(response1.models) == 2  # Excludes davinci

        # With metadata (though base model won't show extra fields)
        response2 = EnhancedTranslationService.translate_with_metadata(models, include_metadata=True)
    ...
    def test_error_handling_in_translation(self):
        ...
        good_model = Model(id="gpt-3.5-turbo", created=1234567890, object="model", owned_by="openai")
...
class TestEdgeCases:
    ...
    def test_timestamp_conversion(self, timestamp, expected_substr):
        ...
        model = Model(id="test-model", created=timestamp, object="model", owned_by="openai")
